...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (waveformInteractionElement && wavesurfer) {
        ...
        waveformInteractionElement.addEventListener('touchstart', (event) => {
            ...
            // --- INICIO FASE 1: Ruta Legacy PC (Limpieza Fase 4) ---
            const isMobileScreen = window.innerWidth <= 768;

            if (!isMobileScreen) {
                // RUTA LEGACY (Solo preservada para PC)
    ...
    const VloitzLaserEngine = (() => {
        ...
        const handleTouchMove = (e) => {
            ...
            if (state.active) {
                ...
                const targetTime = Math.max(0, Math.min(state.duration, proposedTarget));

                state.accumulatedTime = targetTime - state.baseTime;
                ...
                if (Math.abs(frameDelta) >= CONFIG.hapticTickSeconds) {
                    state.lastAccumulatedForHaptics = state.accumulatedTime;
                    if (navigator.vibrate) navigator.vibrate(CONFIG.hapticIntensity);
                }
        ...
        const renderActionPill = (e) => {
            if (document.getElementById('vloitz-laser-actions')) return;
            ...
            // FASE 1 + 3: Posición Matemática Exacta (-25%)
            actionPill.style.top = `${touchY}px`;
            actionPill.style.left = `${touchX}px`;
            ...
            // ==========================================================================
            // 🛡️ EL ÚLTIMO ESLABÓN: PROXY VECTORIAL FANTASMA EN LA PASTILLA
            // ==========================================================================
            let pillStartX = 0;
            let pillStartY = 0;
            let pillIntentLocked = null;
            let proxyActive = false;

            actionPill.addEventListener('touchstart', (ev) => {
                pillStartX = ev.touches[0].clientX;
                pillStartY = ev.touches[0].clientY;
                pillIntentLocked = null;
                proxyActive = false;
            }, {
                passive: true
            });

            actionPill.addEventListener('touchmove', (ev) => {
                if (pillIntentLocked === 'click') return;

                const touch = ev.touches[0];
                const deltaX = Math.abs(touch.clientX - pillStartX);
                const deltaY = Math.abs(touch.clientY - pillStartY);

                if (!pillIntentLocked && (deltaX > 15 || deltaY > 15)) {
                    // Si el arrastre es horizontal, bloqueamos la intención como "drag"
                    if (deltaX > deltaY * 1.5) {
                        pillIntentLocked = 'drag';
                    } else {
                        pillIntentLocked = 'click';
                    }
                }

                if (pillIntentLocked === 'drag') {
                    if (ev.cancelable) ev.preventDefault();

                    // Escondemos la pastilla dejándola en el DOM para no cortar el hilo táctil
                    actionPill.style.opacity = '0';
                    actionPill.style.pointerEvents = 'none';

                    // Buscamos tu contenedor exacto según tu código original
                    const tracklistEl = document.getElementById('current-tracklist');

                    if (tracklistEl) {
                        // Disparamos el primer toque fantasma para despertar el motor láser
                        if (!proxyActive) {
                            proxyActive = true;
                            tracklistEl.dispatchEvent(new TouchEvent('touchstart', {
                                touches: ev.touches,
                        ...
                        // Redirigimos el movimiento en vivo hacia el contenedor de abajo
                        tracklistEl.dispatchEvent(new TouchEvent('touchmove', {
                            touches: ev.touches,
                            changedTouches: ev.changedTouches,
                            bubbles: true
                        }));
            ...
            actionPill.addEventListener('touchend', (ev) => {
                if (pillIntentLocked === 'drag' && proxyActive) {
                    const tracklistEl = document.getElementById('current-tracklist');
                    if (tracklistEl) {
                        // Avisamos al láser que soltamos el dedo
                        tracklistEl.dispatchEvent(new TouchEvent('touchend', {
                            touches: ev.touches,
                            changedTouches: ev.changedTouches,
                            bubbles: true
                        }));
                    }
                    // Destruimos la pastilla fantasma
                    window.vloitzLaserCleanup(false);
                }
            });
            // ==========================================================================

            // ==========================================================================
            // 🛡️ PASTILLA INTELIGENTE: GESTOR DE TRANSICIÓN Y ANTI-SALTO ACCIDENTAL
            // ==========================================================================
            let pillStartX = 0;
            let pillStartY = 0;
            let isDraggingFromPill = false;
            let pillActionTriggered = false;
            ...
            actionPill.addEventListener('touchmove', (ev) => {
                ...
                // Si el usuario mueve el dedo más de 15px y predomina el eje horizontal
                if (!isDraggingFromPill && (deltaX > 15 || deltaY > 15)) {
                    if (deltaX > deltaY * 1.5) {
                        isDraggingFromPill = true;
                        pillActionTriggered = true; // Anulamos cualquier clic o toque en los botones

                        // 1. Destruimos la pastilla anterior al instante
                        window.vloitzLaserCleanup(true);

                        // 2. Despertamos un nuevo ciclo láser directamente en el contenedor del tracklist
                        const tracklistEl = document.getElementById('current-tracklist');
                        if (tracklistEl) {
                            tracklistEl.dispatchEvent(new TouchEvent('touchstart', {
                                touches: ev.touches,
                        ...
                        isDraggingFromPill = true;
                        pillActionTriggered = true; // Si es vertical, bloqueamos los botones para evitar falsos positivos
                    }
                }

                if (isDraggingFromPill) {
                    if (ev.cancelable) ev.preventDefault();
                    const tracklistEl = document.getElementById('current-tracklist');
                    if (tracklistEl) {
                        tracklistEl.dispatchEvent(new TouchEvent('touchmove', {
            ...
            const handleAction = (isConfirm) => (event) => {
                if (pillActionTriggered) return; // 🛑 Si hubo intento de arrastre, bloqueamos el salto por completo
            ...
            confirmBtn.addEventListener('touchend', (e) => {
                if (!isDraggingFromPill) handleAction(true)(e);
            });

            cancelBtn.addEventListener('click', handleAction(false));
            cancelBtn.addEventListener('touchend', (e) => {
                if (!isDraggingFromPill) handleAction(false)(e);
            });
            // ==========================================================================

            actionPill.ontouchstart = actionPill.onclick = (event) => {
        ...
        return {
            init: () => {
                ...
                if (!document.getElementById('vloitz-capsule-styles')) {
                    ...
                    style.innerHTML = `
                        @keyframes vloitz-pill-in {
                            0% { opacity: 0; transform: translate(-50%, -10%) scale(0.92); }
                            100% { opacity: 1; transform: translate(-50%, -25%) scale(1); }
                        }
                        @keyframes vloitz-pill-out {
                            0% { opacity: 1; transform: translate(-50%, -25%) scale(1); }
                            100% { opacity: 0; transform: translate(-50%, -10%) scale(0.92); }
                        }
                        ...
                        #vloitz-laser-actions {
                            ...
                            animation: vloitz-pill-in 0.2s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
                        }
                        #vloitz-laser-actions.closing {
                            animation: vloitz-pill-out 0.15s ease-in forwards !important;
                        }
            ...
            cleanup: (force = false) => {
                ...
                if (!state.sessionActive && !state.active && !document.getElementById('vloitz-laser-actions')) return;

                state.sessionActive = false;
                state.active = false;
                state.accumulatedTime = 0;

                // FASE 3: Animación de salida programada
                const actionPill = document.getElementById('vloitz-laser-actions');
                if (actionPill && !actionPill.classList.contains('closing')) {
                    actionPill.classList.add('closing');
                    setTimeout(() => actionPill.remove(), 200); // Dar tiempo al keyframe
                }
    ...
    const UpdateBannerController = (() => {
        ...
        const createToastDOM = () => {
            ...
            document.getElementById('updateApplyBtn').addEventListener('click', () => {
                if (newWorker) {
