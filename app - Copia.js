...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const TrackActionsController = (() => {
        const copyTrack = (trackName, buttonElement) => {
            ...
            navigator.clipboard.writeText(trackName).then(() => {
                ...
                if (svg) {
                    ...
                    svg.style.stroke = '#1DB954'; // Verde Vloitz de confirmación
                    setTimeout(() => {
        ...
        const searchTrack = (trackName) => {
            ...
            // Mandamos el tráfico a Beatport como jugada Senior
            const beatportUrl = `https://www.beatport.com/search?q=${query}`;
            window.open(beatportUrl, '_blank');
            console.log(`[TrackActions] Buscando en Beatport: ${trackName}`);
        };
    ...
    wavesurfer.on('play', () => {
        ...
        if (!pwaPromptTriggered) {
            ...
            setTimeout(() => {
                // Si pasaron los 60s, comprobamos si podemos mostrar el mensaje
    ...
    currentTracklistElement.addEventListener('click', (e) => {
        ...
        if (target.classList.contains('favorite-btn')) {
            ...
            const title = listItem ? listItem.querySelector('.track-title').textContent : '';
            TrackActionsController.copyTrack(title, target.closest('.btn-copy'));
        } else if (target.closest('.btn-search')) {
            const listItem = target.closest('.current-tracklist-item');
            const title = listItem ? listItem.querySelector('.track-title').textContent : '';
            TrackActionsController.searchTrack(title);
        }
    ...
    const PWA_CONFIG = {
        INITIAL_DELAY: 60000, // <-- 60 segundos exactos después del primer Play
