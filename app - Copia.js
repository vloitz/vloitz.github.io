...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const ShareController = (() => {
        ...
        const openModal = () => {
            ...
            timeLabel.textContent = `${timePrefix}${formatTime(currentTime)}`;
    ...
    // --- INICIO FASE 0: Aislamiento de Variables Táctiles ---
    // Variables Legacy v6 (Se mantendrán hasta el reemplazo final)
    let isDraggingWaveformTouch = false;
    let isHoveringWaveform = false; // <-- NUEVO: Bandera para Preview de Tiempo
    let longTouchTimer = null;
    const LONG_TOUCH_THRESHOLD = 200;
    let wasPlayingBeforeDrag = false; // Para saber si pausar/reanudar

    // Variables Ghost Scrubbing Pro (Preparando Terreno)
    let ghostTouchTimer = null;
    let ghostDragging = false;
    let precacheDebounceTimer = null;
    ...
    if (waveformInteractionElement && wavesurfer) {
        ...
        // --- INICIO FASE 2 y 3: Búsqueda Fantasma y Precarga Cuántica ---
        waveformInteractionElement.addEventListener('touchmove', (e) => {
            if (!e.touches || e.touches.length === 0) return;

            if (ghostDragging) {
                // RUTA MÓVIL: Ghost Scrubbing Pro

                // 1. Bloqueo de scroll nativo para máxima fluidez y control
                if (e.cancelable) e.preventDefault();

                const rect = waveformInteractionElement.getBoundingClientRect();
                const clientX = e.touches[0].clientX;
                const currentProgress = Math.max(0, Math.min(1, (clientX - rect.left) / rect.width));

                // Guardamos la coordenada final para usarla al soltar el dedo (touchend)
                lastCalculatedProgress = currentProgress;

                const duration = wavesurfer.getDuration() || 0;
                const targetTime = currentProgress * duration;

                // 2. Actualización de Interfaz a 60 FPS
                if (window.vloitzHud) {
                    window.vloitzHud.innerText = `${formatTime(targetTime)} | ${formatTime(duration)}`;
                }
                if (window.vloitzGhostPointer) {
                    window.vloitzGhostPointer.style.left = `${currentProgress * 100}%`;
                }

                // 3. FASE 3: Ventana Cuántica (Debounce Táctil)
                clearTimeout(precacheDebounceTimer);
                precacheDebounceTimer = setTimeout(() => {
                    if (typeof PrecacheController !== 'undefined' && PrecacheController.handleInteraction) {
                        PrecacheController.handleInteraction(clientX, rect);
                        console.log(`%c[Quantum Ghost] Freno detectado. Precargando en ${formatTime(targetTime)}`, "color: #00F3FF; font-size: 10px; font-weight: bold;");
                    }
                }, 120); // 120ms de micro-duda humana

            } else {
                // Si el usuario mueve el dedo ANTES de cumplir los 200ms, cancelamos la ilusión (Es un swipe o scroll)
                clearTimeout(ghostTouchTimer);

                // Respetamos tu precarga clásica por movimiento si no estamos en Ghost Scrubbing
                if (!isDraggingWaveformTouch) {
                    const rect = waveformInteractionElement.getBoundingClientRect();
                    if (typeof PrecacheController !== 'undefined' && PrecacheController.handleInteraction) {
        ...
        waveformInteractionElement.addEventListener('touchstart', (event) => {
            ...
            clearTimeout(ghostTouchTimer); // Fase 1: Limpieza del timer fantasma
            ...
            // --- INICIO FASE 1: Bifurcación Táctil (Ghost Scrubbing vs Legacy) ---
            const isMobileScreen = window.innerWidth <= 768;

            if (isMobileScreen) {
                // RUTA GHOST SCRUBBING (MÓVIL)
                ghostTouchTimer = setTimeout(() => {
                    console.log(`%c[Ghost Scrubbing] 👻 Ilusión Activada en ${formattedTouchStartTime}`, "color: #1DB954; font-weight: bold;");
                    ghostDragging = true;
                    // LA MÚSICA NO SE PAUSA

                    if (window.vloitzHud) {
                        const titleEl = document.getElementById('current-track-title');
                        if (titleEl) {
                            // Alineamos el HUD exactamente donde está el título
                            window.vloitzHud.style.top = (titleEl.offsetTop - 2) + 'px';
                            // Desvanecemos el título suavemente
                            titleEl.style.transition = 'opacity 0.2s';
                            titleEl.style.opacity = '0';
                        }
                        window.vloitzHud.style.display = 'block';
                    }
                    if (window.vloitzGhostPointer) {
                        window.vloitzGhostPointer.style.display = 'block';
                        // Posición inicial del fantasma alineada al dedo
                        const wsWrapper = wavesurfer.getWrapper();
                        const rect = wsWrapper.getBoundingClientRect();
                        const clientX = event.touches[0].clientX;
                        const progress = Math.max(0, Math.min(1, (clientX - rect.left) / rect.width));
                        window.vloitzGhostPointer.style.left = `${progress * 100}%`;

                        const totalDur = wavesurfer.getDuration();
                        window.vloitzHud.innerText = `${formatTime(progress * totalDur)} | ${formatTime(totalDur)}`;
                    }
                }, LONG_TOUCH_THRESHOLD);
            } else {
                // RUTA LEGACY (PC)
                longTouchTimer = setTimeout(() => {
                    console.warn(`[Drag v6 Final Merged] ¡TOQUE LARGO DETECTADO (Legacy)! en ${formattedTouchStartTime}`);
        ...
        // --- INICIO FASE 4: El Aterrizaje Cuántico (touchend) ---
        const handleWaveformTapEnd = (event) => {
            console.log(`[Drag v7 Refactored] Evento LOCAL: ${event.type} detectado.`);

            // 1. Limpieza rigurosa de TODOS los temporizadores
            if (longTouchTimer) clearTimeout(longTouchTimer);
            if (ghostTouchTimer) clearTimeout(ghostTouchTimer);
            if (precacheDebounceTimer) clearTimeout(precacheDebounceTimer);
            longTouchTimer = null;
            ghostTouchTimer = null;

            // 2. Ejecución del Salto Fantasma (Solo Móvil)
            if (ghostDragging) {
                ghostDragging = false;
                console.log(`%c[Ghost Scrubbing] 🛬 Aterrizaje consolidado en progreso: ${lastCalculatedProgress.toFixed(4)}`, "color: #1DB954; font-weight: bold;");

                // Ocultar Ilusión Visual (HUD y Puntero) y restaurar Título
                if (window.vloitzHud) {
                    window.vloitzHud.style.display = 'none';
                    const titleEl = document.getElementById('current-track-title');
                    if (titleEl) titleEl.style.opacity = '1';
                }
                if (window.vloitzGhostPointer) window.vloitzGhostPointer.style.display = 'none';

                // Disparo Final al motor de audio
                if (wavesurfer && typeof wavesurfer.seekTo === 'function') {
                    wavesurfer.seekTo(lastCalculatedProgress);

                    // Si el motor ya estaba reproduciendo antes del arrastre, aseguramos la continuidad
                    if (!wavesurfer.isPlaying()) {
                        wavesurfer.play().catch(e => console.warn("Auto-play bloqueado tras Ghost Scrubbing:", e));
                    }
                }
            }
        };
    ...
    // --- INICIO: MOTOR KINETIC BYPASS (Armor Piercing Scroll) ---
    let bypassStartY = 0;
    let bypassIsDragging = false;

    currentTracklistElement.addEventListener('touchstart', (e) => {
        // Solo activamos el motor si el toque cae en el blindaje de botones
        if (e.target.closest('.action-group')) {
            bypassStartY = e.touches[0].clientY;
            bypassIsDragging = true;
        }
    }, {
        passive: true
    });

    currentTracklistElement.addEventListener('touchmove', (e) => {
        if (!bypassIsDragging) return;

        // Calculamos la física del arrastre
        const currentY = e.touches[0].clientY;
        const deltaY = bypassStartY - currentY;

        // Empujamos la página principal, puenteando la lista
        window.scrollBy(0, deltaY);

        // Reseteamos el anclaje para el siguiente micro-movimiento
        bypassStartY = currentY;
    }, {
        passive: true
    });

    currentTracklistElement.addEventListener('touchend', () => bypassIsDragging = false, {
        passive: true
    });
    currentTracklistElement.addEventListener('touchcancel', () => bypassIsDragging = false, {
        passive: true
    });
    // --- FIN: MOTOR KINETIC BYPASS ---
    ...
    const PWA_CONFIG = {
        INITIAL_DELAY: 10000, // <-- 60 segundos exactos después del primer Play
